...
- `bun run deploy:stage` / `bun run deploy:prod` — build, then `scripts/deploy.mjs` patches `dist/server/wrangler.json` and runs `wrangler deploy`. Before either: `.env.local`'s `PUBLIC_ORDER_TOKEN` must equal the **target env's** `ORDER_TOKEN` wrangler secret. Stage and prod have different secrets, build is single-pass, so swap the active line in `.env.local` per cross-env deploy. **`deploy:*` clean-builds first (`rm -rf dist .astro node_modules/.vite`)** because Vite caches the inlined `PUBLIC_ORDER_TOKEN` and will NOT re-bake on a `.env.local`-only change — a dirty build silently re-ships the previous env's token and customer orders fail with `INVALID_TOKEN` (caused a prod incident 2026-06-05). `scripts/deploy.mjs` aborts if `dist/client/` still contains `REPLACE_AT_BUILD_TIME` (`.env.local` missing the var) OR if `.env.local`'s active token isn't found in the client bundle (stale cache slipped through)
